...
**M3a inference is an accepted approximate development screen.** The integrated monthly method passed the
...
Protocol remains strict JSON at `schema_version=1`; studies accept historical v1
and explicit v2 described in the M3b section. Unknown semantic keys,
...
empty selection and unsupported execution scope all fail before work. V2 context
features use the explicit coordinator API above; hypotheses/models remain
instrument-scoped.

### Features, hypotheses and occurrence

A feature declares its ID, version, parameters, dependencies, required prior
bars, initialization and instrument or explicit context scope, and returns aligned float64 values
...
ordinary v2 integration of this method. **M3a was accepted at `9fe7816` after
integration and compatibility review**, for the documented approximate
development-screen scope; the research PASS itself is not milestone acceptance.
...
- **M3a is accepted at `9fe7816` for its approximate development-screen scope.**
