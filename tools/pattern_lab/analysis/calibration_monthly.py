...
**Research driver only.** Ordinary v2 analysis uses the shared numerical kernel
without importing this driver. Integration does not accept the M3a milestone.
...
from .monthly import (
    monthly_jackknife, _balance, _unavailable_candidate, _require_finite,
    MIN_INFORMATIVE_MONTHS, REASON_MONTHS, REASON_DELETION,
    CANDIDATE_REASON_ORDER, DEGENERACY_MULTIPLIER, CONFIDENCE_LEVEL,
)
from .estimator import _evaluate_monthly_candidate


def evaluate_candidate(accumulated: AccumulatedEstimates) -> dict[str, Any]:
    """Compatibility adapter over the shared monthly inference assembly."""
    return _evaluate_monthly_candidate(accumulated)
...
def score_fixture(fixture: CandidateFixture, rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "primary_availability_meets_floor": (
            None if availability is None else bool(availability >= cal.MIN_PRIMARY_AVAILABILITY)
        ),
        ...
        "rate_checks_passed": (
            ...
            if availability is None
                ...
                and availability >= cal.MIN_PRIMARY_AVAILABILITY
            )
        ),
        "effect": {
            "true_lift": fixture.truth,
        ...
        if value >= IMPOSSIBLE_ERRORS:
            return {
                ...
                "proof": (
                    ...
                    f"{cal.ERROR_ENVELOPE} < exact_upper_bound({IMPOSSIBLE_ERRORS}, "
                    f"{fixture.attempts}) = "
    ...
    if unavailable >= IMPOSSIBLE_UNAVAILABLE:
        return {
            ...
            "proof": (
                ...
                f"{(fixture.attempts - IMPOSSIBLE_UNAVAILABLE) / fixture.attempts} < "
                f"{cal.MIN_PRIMARY_AVAILABILITY}; the unavailable count can only grow."
            ),
        }
    return None
...
def candidate_manifest() -> dict[str, Any]:
    ...
    manifest = {
        ...
        "integration": (
            "Ordinary request v2 integrates the shared monthly method. This research driver "
            "remains separate; its PASS is a synthetic-screen decision, not M3a acceptance "
            "or a validated edge. Explicit v1 retains the unvalidated bootstrap."
        ),
        "implementation": artifacts.module_digests(),
        "research_implementation": research_digests(),
        "research_attribution_revision": admission.RESEARCH_ATTRIBUTION_REVISION,
    }
...
def summarize(output_root: Any) -> dict[str, Any]:
    ...
    problems, incomplete = [], []
    verifier_errors = admission.verifier_problems(
        decision={"error_envelope": cal.ERROR_ENVELOPE,
                  "min_primary_availability": cal.MIN_PRIMARY_AVAILABILITY,
                  "rates": list(cal.ACCEPTANCE_RATES), "impossible_errors": IMPOSSIBLE_ERRORS,
                  "impossible_unavailable": IMPOSSIBLE_UNAVAILABLE},
        main=[asdict(f) for f in MAIN_MATRIX], supplementary=[asdict(f) for f in SUPPLEMENTARY_MATRIX],
        max_main_attempts=MAX_MAIN_ATTEMPTS,
    )
    problems.extend(verifier_errors)
    manifest, run = {}, None
    input_hashes = {}
    ...
    for fixture in (() if verifier_errors else fixtures):
        try:
    ...
    if not isinstance(stops, list) or any(not isinstance(item, dict) for item in stops):
        ...
        stops = []
    for stop in (() if verifier_errors else stops):
    ...
    elif manifest.get("research_attribution_revision", 1) == 1:
        limitations.append("Historical research attribution revision 1 predates shared monthly-module extraction; "
                            "its original five-source scope is retained, without a retroactive helper hash.")
    summary = {
        "schema_version": admission.EVIDENCE_SCHEMA_VERSION,
        "plan_version": admission.PLAN_VERSION,
        "decision_policy_version": admission.DECISION_POLICY_VERSION,
        ...
        json.dumps(
            {
                ...
                "integration": (
                    "Research driver only: ordinary v2 integrates the shared monthly method; "
                    "no outcome here accepts M3a or starts M3b."
                ),
            },
            indent=2,
        )
